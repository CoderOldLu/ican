...
def smart_str(s):
    ...
    #                                        errors) for arg in s])
    #         return request.unicode(s).encode(encoding, errors)
...
def notify_verify(post):
    ...
    params = {'partner': settings.ALIPAY_PARTNER, 'notify_id': post.get('notify_id')}

    gateway = 'https://mapi.alipay.com/gateway.do?service=notify_verify&'
    verify_result = urllib.request.urlopen(gateway + urllib.parse.urlencode(params)).read().decode('utf-8')
    if verify_result.lower().strip() == 'true':
        return True
    return False


def get_old_password_md5_style(old_password):
    key = 'drmkey'
    return md5((old_password + key).encode('utf-8')).hexdigest()
